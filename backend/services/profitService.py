class ProfitService:
    ...
    def deleteLowestProfitFlight(self):
        ...
        if candidate["node"] is None:
            return None
        # cancel the node
        self.treeService.descendantsCancelation(
            candidate["node"].getValue().getIdFlight()
        )
        return (
            candidate["node"].getValue().getIdFlight()
        )  # Returns code of the deleted node

    def _findLowestProfitNode(self, node, depth, candidate):
        """
        Private method that traverses the tree and updates the candidate node
        with lower profitability following the tie-breaking rules.
        """
        if node is None:
            return
        ...
        if rent < candidate["rent"]:
            ...
            if depth > candidate["depth"]:
                ...
                if (
                    candidate["node"] is None
                    or flight.getIdFlight() > candidate["node"].getValue().getIdFlight()
                ):
                    update = True
